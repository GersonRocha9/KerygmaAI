...
export const extractDevotionalParts = (text: string | string[] | Record<string, string> | null | undefined): DevotionalParts => {
  ...
  const topicosPtRegex = /TÓPICO (I|II|III|IV|V|VI):\s*([^\n]+)([^]*?)(?=TÓPICO |CONCLUSÃO:|ORAÇÃO:|$)/gi;
  let topicoPtMatch;

  while ((topicoPtMatch = topicosPtRegex.exec(contentStr)) !== null) {
  ...
  const topicosEnReflection = contentStr.match(/Reflection:?\s*([^]*?)(?=Practical Application:|Closing Prayer:|$)/is);
  const topicosEnApplication = contentStr.match(/Practical Application:?\s*([^]*?)(?=Closing Prayer:|$)/is);

  if (topicosEnReflection && topicosEnReflection[1]) {
    result.topicos.push({
      titulo: 'Reflection',
  ...
  if (topicosEnApplication && topicosEnApplication[1]) {
    ...
    if (isNumberedList) {
      ...
      const listItems = applicationContent.split(/\n+/).filter(line => line.trim().length > 0);
  ...
  const conclusaoPtMatch = contentStr.match(/CONCLUSÃO:\s*([^]*?)(?=ORAÇÃO:|$)/is);
  if (conclusaoPtMatch && conclusaoPtMatch[1]) {
  ...
  if (oracaoPtMatch && oracaoPtMatch[1]) {
    result.oracaoParagraphs = oracaoPtMatch[1]
      .split(/\n+/)
      .filter((p: string) => p.trim().length > 0)
      .map((p: string) => cleanMarkdown(p.trim()));
  } else if (oracaoEnMatch && oracaoEnMatch[1]) {
