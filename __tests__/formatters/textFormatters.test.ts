...
describe('extractDevotionalParts', () => {
  describe('formato português', () => {
    ...
    it('deve extrair corretamente o versículo e referência em português', () => {
      ...
      expect(resultado.verse).toBe('Confia no Senhor de todo o teu coração, e não te estribes no teu próprio entendimento.');
    ...
    it('deve extrair corretamente a introdução em português', () => {
      ...
      expect(resultado.introducaoParagraphs[0]).toBe('Este é um texto introdutório em português.');
      expect(resultado.introducaoParagraphs[1]).toBe('Este é o segundo parágrafo da introdução.');
    });
    ...
    it('deve extrair corretamente os tópicos em português', () => {
      ...
      expect(resultado.topicos[0].paragraphs[0]).toBe('Este é o conteúdo do primeiro tópico.');
      expect(resultado.topicos[1].titulo).toBe('Segundo tema');
      expect(resultado.topicos[1].paragraphs[0]).toBe('Este é o conteúdo do segundo tópico.');
    });

    it('deve extrair corretamente a conclusão em português', () => {
      const resultado = extractDevotionalParts(devocionalPt);
      expect(resultado.conclusaoParagraphs).toHaveLength(1);
      expect(resultado.conclusaoParagraphs[0]).toBe('Esta é a conclusão do devocional.');
    });

    it('deve extrair corretamente a oração em português', () => {
      const resultado = extractDevotionalParts(devocionalPt);
      expect(resultado.oracaoParagraphs).toHaveLength(2);
      expect(resultado.oracaoParagraphs[0]).toBe('Senhor, obrigado pela tua palavra.');
  ...
  describe('formato inglês', () => {
    ...
    it('deve extrair corretamente o versículo e referência em inglês', () => {
      ...
      // Versículo
      expect(resultado.verse).toContain('And now these three remain: faith, hope and love');
    ...
    it('deve extrair corretamente a introdução em inglês', () => {
      const resultado = extractDevotionalParts(devocionalEn);
      expect(resultado.introducaoParagraphs).toHaveLength(2);
      expect(resultado.introducaoParagraphs[0]).toContain('In the hustle and bustle of our daily lives');
      expect(resultado.introducaoParagraphs[1]).toContain('The Bible teaches us that love is the foundation');
    });
    ...
    it('deve extrair corretamente os tópicos de reflexão em inglês', () => {
      ...
      if (reflexao) {
        ...
        const conteudoEncontrado = reflexao.paragraphs.some(p =>
          p.includes('Corinthians') || p.includes('love') || p.includes('Christian')
        );
    ...
    it('deve extrair corretamente aplicação prática em inglês', () => {
      ...
      const aplicacao = resultado.topicos.find(t => t.titulo === 'Practical Application');
    ...
    it('deve extrair corretamente a oração em inglês', () => {
      const resultado = extractDevotionalParts(devocionalEn);
      const temTextoEsperado = resultado.oracaoParagraphs.some(p =>
        p.includes('Heavenly Father') || p.includes('thank you')
      );
  ...
  describe('limpeza de marcação markdown', () => {
    it('deve remover corretamente asteriscos duplos (negrito)', () => {
      ...
      const textoCapturado = todasPartes.some(p => p.includes('texto em negrito'));
