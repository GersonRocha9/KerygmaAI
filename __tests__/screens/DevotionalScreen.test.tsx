  ...
  useLanguage: () => ({
    t: (key: string) => {
      const translations: { [key: string]: string } = {
        'devotional.questionPrompt':
          'Qual tema você gostaria de refletir hoje?',
        'devotional.themePlaceholder':
          'ex: paciência, perdão, confiança, esperança...',
        ...
        'devotional.disclaimer':
          'Os devocionais são gerados por IA e podem conter imprecisões.',
        'devotional.howToUseDescription':
          'Escolha um tema e receba um devocional personalizado.',
        'common.appName': 'Kerygma AI',
        'devotional.noContentError':
          'Não foi possível gerar o conteúdo do devocional.',
...
describe('DevotionalScreen', () => {
  ...
  it('deve validar o campo de tema como obrigatório', async () => {
    ...
    await waitFor(() => {
      expect(
        getByText('Por favor, digite um tema para o devocional')
      ).toBeTruthy()
    })
  })
  ...
  it('deve gerar um devocional quando o formulário for submetido com tema válido', async () => {
    ...
    await waitFor(() => {
      const generateDevotional =
        require('@/services/aiService').generateDevotional
      const saveDevotionalToHistory =
        require('@/services/devotionalService').saveDevotionalToHistory
